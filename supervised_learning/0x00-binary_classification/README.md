...
m is the number of examples
Y is a numpy.ndarray with shape (1, m) that contains the correct labels for the input data
Returns the neurons prediction and the cost of the network, respectively
...
b1: The bias for the hidden layer. Upon instantiation, it should be initialized with 0s.
...
The biases of the network should be initialized to 0s and saved in the weights dictionary using the key b{l} where {l} is the hidden layer the bias belongs to
You are allowed to use one loop
```
...
The biases of the network should be initialized to 0s and saved in the __weights dictionary using the key b{l} where {l} is the hidden layer the bias belongs to
Each private attribute should have a corresponding getter function (no setter function).
You are allowed to use one loop
```
